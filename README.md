...
- Access to a configured model such as **`argo-gpt4o`**. Argo models are available on
  Argonne network by setting `ARGO_USER` and `ARGO_API_ENDPOINT` environment variables.
...
- The `-m` flag specifies the model to use. Replace `argo-gpt4o` with another model if
  needed.
